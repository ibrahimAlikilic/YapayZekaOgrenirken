...
* ana kod src içerisinde olduğundan ../input/data/data/yolo.h5 olarak güncellendi
* Keras yerine TensorFlow Keras: Kod artık TensorFlow'un keras API'si ile uyumlu hale getirildi.
* K.get_value() Kaldırıldı: K.get_value() fonksiyonu çıkarıldı ve direkt TensorFlow işlemleri kullanıldı.
* Keras Backend Yerine TensorFlow: Keras backend yerine, tensorflow operasyonları kullanıldı.
anchors_tensor'ın veri tipi dönüştürme: anchors_tensor artık tf.cast() kullanılarak float32 veri tipine dönüştürülüyor.
Bu, çarpma işlemi sırasında veri tipi uyuşmazlığı sorununu çözecektir.
"""
...
class YOLO:
    ...
    def __init__(self, obj_threshold, nms_threshold):
        ...
        self._yolo = load_model("../../../../../Masaüstü/YOLOlar/yolo.h5")
    ...
    def _process_feats(self, out, anchors, mask):
        ...
        anchors_tensor = tf.cast(tf.reshape(tf.Variable(anchors), [1, 1, len(anchors), 2]), tf.float32)
        out = out[0]

        box_xy = tf.sigmoid(out[..., :2])
        box_wh = tf.exp(out[..., 2:4]) * anchors_tensor
        box_confidence = tf.sigmoid(out[..., 4:5])
